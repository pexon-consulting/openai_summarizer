...
def get_blogposts(url, username, token, limit) -> ConfluenceSearchResponse:
    """
    ...
    ----------
    url : str
        The base URL for the Confluence API.
    username : str
        The username for the Confluence API.
    token : str
        The authentication token for the Confluence API.
    limit : int
    ...
    api_url = f"{url}/rest/api/content/search?cql=type%20in%20(blogpost)%20order%20by%20created%20desc&limit={limit}&expand=body.storage"
